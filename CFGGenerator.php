...
class CFGGenerator{
    ...
    /*
     * 处理Cavy中public $models= 'user','userhelp';引入的文件
     */
	private  function cavyPropertyHandler($node,$block,$fileSummary){
		...
		foreach ($nodes as $node){
			...
			switch ($node->getType()){
				...
				case 'Expr_Eval':
					...
					break ;
  /*
                //处理类中全局赋值，主要用于cavy的model引入
                case 'Stmt_Property':
                    $this->cavyPropertyHandler($node, $block, $this->fileSummary);
                    break;
*/
				default:
				    $traverser = new PhpParser\NodeTraverser;
	...
	public function CFGBuilder($nodes,$condition,$pEntryBlock,$pNextBlock){
		//此文件的fileSummary
		global $JUMP_STATEMENT,$LOOP_STATEMENT,$STOP_STATEMENT,$RETURN_STATEMENT,$CAVY_CLASS ;
		...
		foreach($nodes as $node){
            ...
            //添加对cavy的识别
            if(!isset($GLOBALS['IS_CAVY']) && $node->getType() === 'Stmt_Class' && NodeUtils::getNodeClassFirstExtendsName($node) === 'Cavy_Controller_Action') {
                $GLOBALS['IS_CAVY'] = true;
            }

            if($GLOBALS['IS_CAVY'] && $node->getType() === "Stmt_Property"){
                $traverser = new PhpParser\NodeTraverser();
                $propertyPropertyVisitor = new PropertyPropertyVistor();
                $traverser->addVisitor($propertyPropertyVisitor);
                $traverser->traverse(array($node));
                foreach($propertyPropertyVisitor->includeFiles as $includeFile){
                    $this->fileSummary->addIncludeToMap($includeFile);
                }
               // echo $propertyPropertyVisitor->includeFiles;
			...
			//如果节点是return
				...
				return $currBlock ;
            //节点是cavy的类，可以继续分析
			}elseif( $GLOBALS['IS_CAVY'] && (in_array($node->getType(),$CAVY_CLASS))){
                $this->simulate($currBlock);
                $nextBlock = new BasicBlock();
                $this->CFGBuilder($node->stmts,NULL, $currBlock, $nextBlock);
                $currBlock = $nextBlock;
            }elseif($GLOBALS['IS_CAVY'] && ($node->getType() === 'Stmt_ClassMethod'){
                $this->simulate($currBlock);
                $nextBlock = new BasicBlock() ;
                $this->CFGBuilder($node->stmts, NULL, $currBlock, $nextBlock) ;
                $currBlock = $nextBlock ;
...
class PropertyPropertyVistor extends PhpParser\NodeVisitorAbstract{
    public $isModel = false;
    public $includeFiles = array();
    public $includeModel = array();

    public function leaveNode(Node $node){
        if($node instanceof Node\Stmt\PropertyProperty && $node->name === "models"){
            $this->isModel = true;
            $value = SymbolUtils::getSymbolByNode($node->default)->getValue();
            $values = explode(",",$value);
            foreach($values as $value){
                array_push($this->includeFiles,"../models/".ucfirst($value).".php");
                array_push($this->includeModel,ucfirst($value));
            }
