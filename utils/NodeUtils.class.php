...
class NodeUtils{
    ...
    /**
     * 给定一个节点，返回该节点的proper信息
     */
    public static function getPropertyName($node){
        if (!$node instanceof Node){
            return null;
        }
    ...
    public static function getNodeFunctionName($node){
        ...
        switch ($type) {
            ...
            case "Expr_MethodCall":           
                ...
                break;
            //class::static function(),return class:method
    ...
    /**
     * 给定一个节点，返回该节点对应class Extends name,
     * class HttpRpcController extends Cavy_Controller_Action
     * @param Node $node
     * @return class name
     */
    public static function getNodeClassFirstExtendsName($node){
        if (!$node instanceof Node){
            return null;
        }
        $type = $node->getType();
        switch ($type) {
            //class define
            case "Stmt_Class":
                if ($node->extends && $node->extends->parts){
                    return $node->extends->parts[0];
                }
                break;
            //new class
            case "Expr_New"://fix me
                if ($node->extends && $node->extends->parts){
                    return $node->extends->parts[0];
                }
                break;
...
class PropertyVisitor extends Phpparser\NodeVisitorAbstract{
    public $string = null;
    public function leaveNode(Node $node){
        $type = $node->getType();
        if ($type === "Stmt_Property"){
